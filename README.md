# Clash Guardian - Clash Verge 守护进程

一个轻量级的 Windows 系统托盘应用，用于自动监控和维护 Clash Verge 代理客户端的稳定运行。

## ✨ 功能特性

### 🔍 实时监控
- **进程监控** - 每 8 秒检测 mihomo 核心进程状态
- **内存监控** - 监控内存占用，防止内存泄漏
- **代理连通性测试** - 通过 Google 204 接口测试代理是否正常
- **TCP 连接统计** - 监控 TIME_WAIT、ESTABLISHED、CLOSE_WAIT 连接数

### ⚡ 自动恢复
- **进程崩溃自动重启** - 检测到进程不存在时自动启动 Clash Verge
- **内存过高自动重启** - 内存超过 70MB 阈值自动重启
- **连接泄漏自动重启** - CLOSE_WAIT 连接超过 20 个时自动重启
- **智能节点切换** - 代理连续无响应时自动切换到延迟最低的可用节点

### 📊 数据记录
- **监控日志** - 记录所有重启、切换事件到 `guardian.log`
...
```csv
Time,ProxyOK,MemMB,Handles,TimeWait,Established,CloseWait,Node,Event
```
