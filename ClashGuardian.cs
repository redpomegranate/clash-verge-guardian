...
{ 
    ...
    void InitializeTrayIcon() {
        ...
        menu.Items.Add("检查更新", null, delegate { CheckForUpdate(false); });
        menu.Items.Add("-");
    ...
    void CheckForUpdate(bool silent) {
        ThreadPool.QueueUserWorkItem(_ => {
            try {
                ...
                // 解析版本号（tag_name 格式为 "v0.0.4"）
                string latestVersion = GetJsonValue(json, "tag_name", "").TrimStart('v');
                if (string.IsNullOrEmpty(latestVersion)) return;

                if (CompareVersions(latestVersion, APP_VERSION) > 0) {
                    // 找到 .exe 下载地址
                    string downloadUrl = ExtractAssetUrl(json, ".exe");
                    if (string.IsNullOrEmpty(downloadUrl)) {
                        if (!silent) this.BeginInvoke((Action)(() =>
                            MessageBox.Show("新版本 v" + latestVersion + " 未找到可下载的 exe 文件\n请前往 GitHub 手动下载",
                    ...
                    this.BeginInvoke((Action)(() => {
                        DialogResult result = MessageBox.Show(
                            "发现新版本 v" + latestVersion + "（当前 v" + APP_VERSION + "）\n\n是否立即下载更新？",
                            "Clash Guardian 更新",
                            MessageBoxButtons.YesNo, MessageBoxIcon.Information);
                        if (result == DialogResult.Yes) {
                            ThreadPool.QueueUserWorkItem(__ => DownloadAndUpdate(downloadUrl, latestVersion));
                        }
                    }));
                } else if (!silent) {
                    this.BeginInvoke((Action)(() =>
                        MessageBox.Show("当前已是最新版本 v" + APP_VERSION, "检查更新",
    ...
    // 从 GitHub Release JSON 中提取指定后缀的 asset 下载地址
    string ExtractAssetUrl(string json, string suffix) {
        string key = "\"browser_download_url\"";
        int pos = 0;
        while ((pos = json.IndexOf(key, pos)) >= 0) {
            pos += key.Length;
            ...
            if (pos < json.Length && json[pos] == '"') {
                ...
                if (end > pos) {
                    string assetUrl = json.Substring(pos, end - pos);
                    if (assetUrl.EndsWith(suffix, StringComparison.OrdinalIgnoreCase)) {
                        return assetUrl;
                    }
                    pos = end;
                }
    ...
    void DownloadAndUpdate(string downloadUrl, string newVersion) {
        try {
            this.BeginInvoke((Action)(() => {
                logLabel.Text = "最近事件:  正在下载 v" + newVersion + " ...";
                logLabel.ForeColor = COLOR_CYAN;
            }));

            ServicePointManager.SecurityProtocol = (SecurityProtocolType)3072;

            string exePath = Application.ExecutablePath;
            string newExePath = exePath + ".update";
            ...
            Log("更新下载完成: v" + newVersion + " (" + (fi.Length / 1024) + "KB)");

            // 文件热替换（Windows 允许重命名正在运行的 exe）
            File.Move(exePath, oldExePath);       // 当前 exe → .old
            File.Move(newExePath, exePath);        // .update → 当前 exe

            // 启动新进程（传入旧 PID，新进程会等待旧进程退出）
